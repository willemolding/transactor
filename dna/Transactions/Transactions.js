...
function validateTransaction(entry,balanceFunc,params) {
    ...
    if (entry.from === params.committer) balance -= entry.amount;
...
function validateCommit (entryName, entry, header, pkg, sources) {
    switch (entryName) {
    case "transaction":
        return validateTransaction(entry,_getBalance,{committer:getMe()});
    case "preauth":
...
function getChainBalance(creditLimit,params) {
    ...
    for (var i=1;i<headers.length;i++) {
        ...
        if (type==="preauth" || type=="transaction") {
            ...
            balance = adjustBalance(balance,headers[i].Type,entry,committer);
...
function validatePut (entryName, entry, header, pkg, sources) {
    var committer = sources[0];
    switch (entryName) {
    case "transaction":
        return validateTransaction(entry,getChainBalance,{chain:pkg.Chain,committer:committer});
    case "preauth":
        return validatePreauth(entry,getChainBalance,{chain:pkg.Chain,committer:committer});
    default:
        // invalid entry name!!
        return false;
    }
 ...
 */
function preauthCreate(preauthInvoice) {
    return commit('preauthInvoice', preauthInvoice);
}

/**
 * cancel authorise spending on a particular proposal
 * this function will only validate if the propoal has completed and didn't pass
 * @param {object} parameters - An object with the following properties:
 *    {string} preauth - hash of the previous preauth
 * @return {string} the preauth cancel entry hash
 */
...
function adjustBalance(balance,type,entry,committer){
    switch(type) {
    case "transaction":
        ...
        if (entry.hasOwnProperty("preauth") && (from == entry.to)) {
            ...
            if (from === committer) balance -= amount;
            else balance += amount;
        }
        break;
    case "preauth":
...
function _getBalance(creditLimit) {
    ...
    for (var i=1;i<entries.length;i++) {
        var entry = entries[i];
        var type = entry.hasOwnProperty("attachedInvoice")?"preauth":"transaction";
        balance = adjustBalance(balance,type,entry,committer);
        if (balance < creditLimit) {
            break;
        }
    }
...
function getCreditLimit() {
  //Call the Credit Zome to calculate limit for entity
